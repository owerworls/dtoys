...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ($this->request->get['user_token'] == $this->session->data['user_token'])) {
			...
			if ($this->user->hasPermission('access', 'catalog/import')) {
				$catalog[] = array(
					'name'	   => 'Импорт',
			...
			if ($this->user->hasPermission('access', 'catalog/pricing')) {
				$catalog[] = array(
					'name'	   => 'Ценообразование',
					'href'     => $this->url->link('catalog/pricing', 'user_token=' . $this->session->data['user_token'], true),
					'children' => array()
				);
			}
